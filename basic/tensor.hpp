...
// ==========================================================================
//  The Tensor class — n-dimensional arrays
// ==========================================================================
//
// A flat buffer plus shape and strides, with reshape / permute / slice as
// METADATA operations that alias rather than copy. Contraction reduces to a
// GEMM by rolling the contracted axes to the end. Shares mstore's allocator
// and thresholds with Matrix, so a seed and a parallel threshold mean the same
// thing in both.
//
...
public:
    ...
    // ── A one-element tensor IS a scalar ───────────────────────────────────
    // The same conversion Matrix has, and for the same reasons — a full
    // contraction, a reduction of a vector, or a logical index that selects one
    // element all land here, and .at({0}) on every one of them is friction with
    // no purpose.
    //
    //     Tensor<double> e = contract(x, x, 1);   // a full contraction: one element
    //     double energy = e;                      // and now just a number
    //
    // Guarded against bool for the reason spelled out on Matrix's version: an
    // implicit operator bool on a mask silently turns `if (mask)` from a
    // compile error into a runtime throw. The static_assert keeps that a
    // compile error, with a message that says what to use instead.
    operator datatype() const {
        static_assert(!std::is_same<datatype, bool>::value,
                      "Tensor<bool> is a MASK, and does not convert to bool. `if (mask)` "
                      "has no single answer — ask .any() or .all(); count with .nnz().");
        if (size_ != 1)
            throw std::logic_error(
                "a Tensor converts to a scalar only when it holds exactly one element, "
                "but this one has " + std::to_string(size_) + " (shape " +
                shapeStr(shape_) + "). Index it, or reduce it first.");
        Tensor c = contiguous();
        return c.data_[0];
    }
...
public:
    ...
    datatype prod() const {
        if (size_ == 0) throw std::invalid_argument("prod: tensor is empty");
        Tensor a = contiguous();
        datatype p = datatype(1);
        for (long i = 0; i < a.size_; i++) p *= a.data_[i];
        return p;
    }

    // Frobenius norm — the square root of the sum of squared magnitudes, which
    // is the only norm that generalises to rank > 2 without picking an axis.
    double norm() const {
        Tensor a = contiguous();
        double acc = 0.0;
        for (long i = 0; i < a.size_; i++) acc += magnitudeSq(a.data_[i]);
        return std::sqrt(acc);
    }

    double var(bool sample = false) const {
        if (size_ == 0) throw std::invalid_argument("var: tensor is empty");
        if (sample && size_ < 2)
        ...
        Tensor a = contiguous();
        double acc = 0.0;
        for (long i = 0; i < a.size_; i++) {
            const double d = magnitudeSigned(a.data_[i]) - m;
            acc += d * d;
        }
        return acc / double(sample ? size_ - 1 : size_);
    }
    double stddev(bool sample = false) const { return std::sqrt(var(sample)); }

    // ── Reductions along ONE axis, which the axis is dropped by ────────────
    // Matrix spells its axis ROW or COL because it only ever has two. A tensor
    // has as many as it has dimensions, so the axis is its INDEX — sum(0) is
    // down the first dimension, sum(rank()-1) along the last.
    Tensor prod(long axis) const { return reduceAxis(axis, "prod", 1); }
    Tensor min(long axis) const  { return reduceAxis(axis, "min", 2); }
    Tensor max(long axis) const  { return reduceAxis(axis, "max", 3); }

    Tensor<double> mean(long axis) const {
        const long a = checkAxis(axis, "mean");
        Tensor s = sum(a);
        const double denom = double(shape_[(std::size_t)a]);
        Tensor<double> out(s.shape());
        for (long i = 0; i < s.size(); i++) out[i] = magnitudeSigned(s[i]) / denom;
        return out;
    }

    // ── Scans along one axis, which KEEP it ───────────────────────────────
    Tensor cumsum(long axis) const { return scanAxis(axis, "cumsum", true); }
    Tensor cumprod(long axis) const { return scanAxis(axis, "cumprod", false); }

    // Successive differences along an axis. Shortens it by one, as Matrix's
    // diff() and MATLAB's do.
    Tensor diff(long axis) const {
        const long a = checkAxis(axis, "diff");
        if (shape_[(std::size_t)a] < 2)
            throw std::invalid_argument("diff: axis " + std::to_string(a) + " has length " +
                                        std::to_string(shape_[(std::size_t)a]) +
                                        ", needs at least 2");
        std::vector<long> order = axisLast(a);
        Tensor rolled = permute(order).contiguous();
        std::vector<long> outShape = rolled.shape_;
        outShape.back() -= 1;
        Tensor out(outShape);
        const long run = rolled.shape_.back(), orun = run - 1;
        const long rows = rolled.size_ / run;
        for (long i = 0; i < rows; i++)
            for (long j = 0; j < orun; j++)
                out.data_[i * orun + j] = rolled.data_[i * run + j + 1] - rolled.data_[i * run + j];
        return out.permute(invPerm(order));
    }

    // ── Element-wise maths ────────────────────────────────────────────────
    // The same set Matrix has, spelled the same way, because the member dot IS
    // the element-wise form: t.sin() is sin applied to every entry.
    Tensor abs() const   { return mapElems([](const datatype& x) { return datatype(std::abs(x)); }); }
    Tensor sqrt() const  { return mapElems([](const datatype& x) { return std::sqrt(x); }); }
    Tensor exp() const   { return mapElems([](const datatype& x) { return std::exp(x); }); }
    Tensor ln() const    { return mapElems([](const datatype& x) { return std::log(x); }); }
    Tensor lg() const    { return mapElems([](const datatype& x) { return std::log2(x); }); }
    Tensor log10() const { return mapElems([](const datatype& x) { return std::log10(x); }); }
    Tensor sin() const   { return mapElems([](const datatype& x) { return std::sin(x); }); }
    Tensor cos() const   { return mapElems([](const datatype& x) { return std::cos(x); }); }
    Tensor tan() const   { return mapElems([](const datatype& x) { return std::tan(x); }); }
    Tensor sinh() const  { return mapElems([](const datatype& x) { return std::sinh(x); }); }
    Tensor cosh() const  { return mapElems([](const datatype& x) { return std::cosh(x); }); }
    Tensor tanh() const  { return mapElems([](const datatype& x) { return std::tanh(x); }); }
    Tensor asin() const  { return mapElems([](const datatype& x) { return std::asin(x); }); }
    Tensor acos() const  { return mapElems([](const datatype& x) { return std::acos(x); }); }
    Tensor atan() const  { return mapElems([](const datatype& x) { return std::atan(x); }); }
    Tensor asinh() const { return mapElems([](const datatype& x) { return std::asinh(x); }); }
    Tensor acosh() const { return mapElems([](const datatype& x) { return std::acosh(x); }); }
    Tensor atanh() const { return mapElems([](const datatype& x) { return std::atanh(x); }); }
    Tensor conj() const {
        return mapElems([](const datatype& x) {
            if constexpr (is_complex<datatype>::value) return std::conj(x);
            else return x;
        });
    }
    template <typename scalar>
    Tensor pow(scalar p) const {
        return mapElems([p](const datatype& x) { return datatype(std::pow(x, datatype(p))); });
    }
    template <typename scalar>
    Tensor log(scalar base) const {
        const double lb = std::log(double(std::real(base)));
        return mapElems([lb](const datatype& x) { return std::log(x) / lb; });
    }

    // These need an ordering or a whole-number part, and std::complex has
    // neither — the same refusal Matrix makes, for the same reason.
    Tensor sign() const {
        return mapElems([](const datatype& x) {
            if constexpr (is_complex<datatype>::value) {
                const double m = std::abs(x);
                return (m == 0.0) ? datatype(0) : datatype(x / m);
            } else {
                return datatype((x > datatype(0)) - (x < datatype(0)));
            }
        });
    }
    Tensor floor() const {
        static_assert(!is_complex<datatype>::value,
                      "floor/ceil/round/fix/expm1/log1p have no complex meaning. "
                      "Apply them to .real() / .imag() / .abs(), or use sign(), "
                      "which is defined for complex.");
        return mapElems([](const datatype& x) {
            if constexpr (is_complex<datatype>::value) return x;
            else return datatype(std::floor(x));
        });
    }
    Tensor ceil() const {
        static_assert(!is_complex<datatype>::value,
        ...
        return mapElems([](const datatype& x) {
            if constexpr (is_complex<datatype>::value) return x;
            else return datatype(std::ceil(x));
        });
    }
    Tensor round() const {
        static_assert(!is_complex<datatype>::value,
        ...
        return mapElems([](const datatype& x) {
            if constexpr (is_complex<datatype>::value) return x;
            else return datatype(std::round(x));
        });
    }
    Tensor fix() const {
        static_assert(!is_complex<datatype>::value,
        ...
        return mapElems([](const datatype& x) {
            if constexpr (is_complex<datatype>::value) return x;
            else return datatype(std::trunc(x));
        });
    }
    Tensor expm1() const {
        static_assert(!is_complex<datatype>::value,
        ...
        return mapElems([](const datatype& x) {
            if constexpr (is_complex<datatype>::value) return x;
            else return datatype(std::expm1(x));
        });
    }
    Tensor log1p() const {
        static_assert(!is_complex<datatype>::value,
        ...
        return mapElems([](const datatype& x) {
            if constexpr (is_complex<datatype>::value) return x;
            else return datatype(std::log1p(x));
        });
    }
    ...
    // ── Printing ────────────────────────────────────────────────────────
    //
    // The same interface Matrix has, and the same formats. Rank is what makes
    // a tensor different here:
    //   Pretty       prints slice by slice, which is the only readable layout
    //   JSON/NumPy   NEST to match the shape, so the structure survives
    //   CSV/TSV/Plain/Markdown are 2-D formats, so a rank > 2 tensor is
    //                flattened to (product of leading axes) x (last axis) and a
    //                `# shape (...)` line is written first — without it the
    //                flattening would not be recoverable.
    void print(std::ostream& os, const matio::Opts& oIn) const {
        matio::Opts o = oIn;
        if (o.fmt == matio::Fmt::Auto) o.fmt = matio::Fmt::Pretty;
        if (o.fmt == matio::Fmt::Pretty) {
            os << toString(o.precision) << '\n';
            return;
        }
        Tensor c = contiguous();
        if (o.fmt == matio::Fmt::JSON || o.fmt == matio::Fmt::NumPy) {
            const bool np = (o.fmt == matio::Fmt::NumPy);
            if (np && !o.name.empty()) os << o.name << " = ";
            if (np) os << "np.array(";
            const std::vector<long> st = rowMajorStrides(c.shape_);
            std::function<void(long, long)> rec = [&](long dim, long off) {
                os << '[';
                const long n = c.shape_[(std::size_t)dim];
                for (long k = 0; k < n; k++) {
                    if (dim + 1 == c.rank()) os << matio::value(c.data_[off + k], o);
                    else rec(dim + 1, off + k * st[(std::size_t)dim]);
                    if (k + 1 < n) os << ", ";
                }
                os << ']';
            };
            if (c.rank() == 0) os << matio::value(c.data_[0], o);
            else rec(0, 0);
            if (np) os << ')';
            os << '\n';
            return;
        }
        const long cols = c.shape_.empty() ? 1 : c.shape_.back();
        const long rows = (cols > 0) ? c.size_ / cols : 0;
        if (c.rank() > 2 && o.fmt != matio::Fmt::Markdown)
            os << "# shape " << shapeStr(c.shape_) << '\n';
        else if (c.rank() > 2)
            os << "shape " << shapeStr(c.shape_) << "\n\n";
        matio::table(os, rows, cols,
                     [&](long i, long j) { return c.data_[i * cols + j]; }, o);
        os << '\n';
    }
    ...
    // Write to a file; with no format given the extension decides.
    void save(const std::string& path, matio::Opts o = {}) const {
        matio::saveWith(path, o, [&](std::ostream& f, const matio::Opts& r) { print(f, r); });
    }
...
private:
    ...
    // Element-wise map, parallel above the same threshold Matrix uses.
    template <class F>
    Tensor mapElems(F f) const {
        Tensor a = contiguous();
        Tensor out(a.shape_);
        const datatype* MATRIXCPP_RESTRICT src = a.data_;
        datatype* MATRIXCPP_RESTRICT dst = out.data_;
        mstore::forEachIndex(a.size_, [=](long i) { dst[i] = f(src[i]); });
        return out;
    }

    // The permutation that moves `a` to the end, and its inverse.
    std::vector<long> axisLast(long a) const {
        std::vector<long> order;
        for (long k = 0; k < rank(); k++)
            if (k != a) order.push_back(k);
        order.push_back(a);
        return order;
    }
    static std::vector<long> invPerm(const std::vector<long>& order) {
        std::vector<long> inv(order.size());
        for (std::size_t k = 0; k < order.size(); k++) inv[(std::size_t)order[k]] = (long)k;
        return inv;
    }

    // kind: 1 = product, 2 = min, 3 = max. The axis is dropped.
    Tensor reduceAxis(long axis, const char* who, int kind) const {
        const long a = checkAxis(axis, who);
        if (kind != 1) requireOrdered();
        Tensor rolled = permute(axisLast(a)).contiguous();
        std::vector<long> outShape(rolled.shape_.begin(), rolled.shape_.end() - 1);
        ...
        const long rows = rolled.size_ / run;
        for (long i = 0; i < rows; i++) {
            const datatype* MATRIXCPP_RESTRICT r = rolled.data_ + i * run;
            datatype acc = (kind == 1) ? datatype(1) : r[0];
            for (long j = (kind == 1 ? 0 : 1); j < run; j++) {
                if (kind == 1) acc *= r[j];
                else if (kind == 2) { if (magnitudeSigned(r[j]) < magnitudeSigned(acc)) acc = r[j]; }
                else               { if (magnitudeSigned(r[j]) > magnitudeSigned(acc)) acc = r[j]; }
            }
            out.data_[i] = acc;
        }
        return out;
    ...
    // Running scan along an axis, which KEEPS it.
    Tensor scanAxis(long axis, const char* who, bool adding) const {
        const long a = checkAxis(axis, who);
        std::vector<long> order = axisLast(a);
        Tensor rolled = permute(order).contiguous();
        Tensor out(rolled.shape_);
        const long run = rolled.shape_.back();
        const long rows = rolled.size_ / run;
        for (long i = 0; i < rows; i++) {
            const datatype* MATRIXCPP_RESTRICT r = rolled.data_ + i * run;
            datatype* MATRIXCPP_RESTRICT w = out.data_ + i * run;
            datatype acc = adding ? datatype(0) : datatype(1);
            for (long j = 0; j < run; j++) {
                acc = adding ? datatype(acc + r[j]) : datatype(acc * r[j]);
                w[j] = acc;
            }
        }
        return out.permute(invPerm(order));
